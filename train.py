...
group.add_argument('--lr', type=float, default=5e-4, metavar='LR',
                    help='learning rate (default: 5e-4 for ImageNet-100 scratch)')
...
def main():
    ...
    # ============================================================
    # Scratch Training Safety Check for ImageNet-100
    # ============================================================
    if args.local_rank == 0:
        _logger.info("========== Scratch Training Check ==========")
        _logger.info(f"model              = {args.model}")
        _logger.info(f"pretrained         = {args.pretrained}")
        _logger.info(f"initial_checkpoint = {args.initial_checkpoint}")
        _logger.info(f"resume             = {args.resume}")
        _logger.info(f"num_classes        = {args.num_classes}")
        _logger.info(f"data_dir           = {args.data_dir}")
        _logger.info(f"train_split        = {args.train_split}")
        _logger.info(f"val_split          = {args.val_split}")
        _logger.info(f"epochs             = {args.epochs}")
        _logger.info(f"lr                 = {args.lr}")
        _logger.info(f"batch_size         = {args.batch_size}")
        _logger.info(f"grad_accum_steps   = {args.grad_accum_steps}")
        _logger.info(f"sched              = {args.sched}")
        _logger.info(f"warmup_epochs      = {args.warmup_epochs}")
        _logger.info(f"mixup              = {args.mixup}")
        _logger.info(f"cutmix             = {args.cutmix}")
        _logger.info(f"drop_path          = {args.drop_path}")
        _logger.info("===========================================")

    assert not args.pretrained, "Scratch training should NOT use --pretrained."
    assert args.initial_checkpoint == "", "Scratch training should NOT use --initial-checkpoint."
    assert args.resume == "", "Scratch training should NOT use --resume."
    assert args.num_classes == 100, "ImageNet-100 scratch training should use --num-classes 100."
